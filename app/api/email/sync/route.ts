        ...
        for (const account of accounts) {
            ...
            try {
                ...
                try {
                    ...
                    // Only fetch the last 50 if it's the first sync
                    const uidsToFetch = account.last_sync_at ? uids : uids.slice(-50);
                    ...
                    results.push({ account: account.name, count: uidsToFetch.length });
                } finally {
                    lock.release();
                }
                await client.logout();
